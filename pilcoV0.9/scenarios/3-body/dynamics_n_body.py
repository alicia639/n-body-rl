...
@mfunction("dz")
def dynamics_cp(t=None, z=None, f=None):
    #% Code
    ...
    if nargin == 3:
        dz = zeros(4, 1)
        dz(1).lvalue = z(2)
        dz(2).lvalue = (2 * m * l * z(3) ** 2 * sin(z(4)) + 3 * m * g * sin(z(4)) * cos(z(4)) + 4 * f(t) - 4 * b * z(2)) / (4 * (M + m) - 3 * m * cos(z(4)) ** 2)
        dz(3).lvalue = (-3 * m * l * z(3) ** 2 * sin(z(4)) * cos(z(4)) - 6 * (M + m) * g * sin(z(4)) - 6 * (f(t) - b * z(2)) * cos(z(4))) / (4 * l * (m + M) - 3 * m * l * cos(z(4)) ** 2)
        dz(4).lvalue = z(3)
    else:
