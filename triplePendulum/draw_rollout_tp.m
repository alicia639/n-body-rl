...
for r = 1:size(xx,1)
  ...
  if exist('j','var') && ~isempty(M{j})
    draw_tp(latent{j}(r,4), latent{j}(r,5), latent{j}(r,6), latent{j}(r,end-2), latent{j}(r,end-1), ...
      latent{j}(r,end), cost,  ...
      ['trial # ' num2str(j+J) ', T=' num2str(H*dt) ' sec'], ...
      ['total experience (after this trial): ' num2str(dt*size(x,1)) ...
      ' sec'], M{j}(:,r), Sigma{j}(:,:,r));
  else
    draw_tp(latent{jj}(r,4), latent{jj}(r,5), latent{jj}(r,6), latent{jj}(r,end-2), latent{jj}(r,end-1), ...
